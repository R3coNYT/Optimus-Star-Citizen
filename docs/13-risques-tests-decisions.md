...
| **D36** | **La capture de touche lit le scancode**, jamais le code virtuel | `GetAsyncKeyState` ou `KeyEventArgs`, plus simples | **Décidé le 2026-08-25.** C'est D19 vue depuis l'autre bout : sur AZERTY, la touche marquée « A » porte le code virtuel `A` mais occupe la position US `Q`, seule connue de Star Citizen et de l'injection. Capturer par le code virtuel ferait enregistrer une touche pour en presser une autre — l'assignation paraîtrait juste et ne marcherait pas, ce qui est le pire des deux mondes. Un hook `WH_KEYBOARD_LL` livre le scancode brut ; les modificateurs, eux, se lisent par code virtuel sans risque, « Ctrl gauche est-il enfoncé » ne dépendant d'aucune disposition |
